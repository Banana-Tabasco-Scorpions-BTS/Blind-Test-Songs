...
const { getCurrentGame, getTrack, incrementRound, incrementScore, getResult } =
  globalModel;

const { sanitiseInput, sanitiseAnswer } = guessModel;
...
router.post("/", async (req: Request, res: Response) => {
  ...
  if (clientGameID === undefined || clientGuess === undefined) {
    return res.status(400).send({
      error:
        "Expected a request body containing: { gameID: '<your_game_id_num>', guess: '<your_guess_str>' }",
    });
  }
  ...
  if (!currentGame) {
    return res
      .status(404)
      .send({ error: "Game not found. We searched real hard... promise." });
  }
